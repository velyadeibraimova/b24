<?php
use \Bitrix\Main\Localization\Loc;
/** @var array $doctor */
/** @var array $procedures */
?>

<h1 style="text-align:center;"><a href="/doctors/"><?= Loc::getMessage('DOCTORS') ?></a></h1>
<br/>
<br/>
<form method="post" class="doctor-add-form" action="/doctors/<?= $doctor['SLUG'] ?>/">
    <input type="hidden" name="_method" value="PUT">
    <input type="hidden" name="ID" value="<?= $doctor['IBLOCK_ELEMENT_ID'] ?>">
    <input type="text" name="SLUG" value="<?= $doctor['SLUG'] ?>">
    <input type="text" name="LASTNAME" value="<?= $doctor['LASTNAME'] ?>">
    <input type="text" name="FIRSTNAME" value="<?= $doctor['FIRSTNAME'] ?>">
    <input type="text" name="PATRONYMIC" value="<?= $doctor['PATRONYMIC'] ?>">
    <select name="PROCS[]" multiple size="10">
        <?php foreach ($procedures as $procedure): ?>
            <?php $selected = in_array($procedure['ID'], $doctor['PROCS_IDS']) ? 'selected' : '' ?>
            <option value="<?= $procedure['ID'] ?>" <?= $selected ?>><?= $procedure['NAME'] ?></option>
        <?php endforeach; ?>
    </select>
    <input type="submit" value="<?= Loc::getMessage('FORM_SUBMIT') ?>">
</form>